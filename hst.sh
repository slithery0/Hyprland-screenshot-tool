#!/bin/bash
FORMAT="png"
NAME="Screenshot_from_$(date "+%d-%m-%y %H:%M:%S")"
...
case $1 in
  '-e' ) grim "${PICTPATH}${OUTPUT}" ;;
  ''| '-s' ) grim -g "$(slurp -d)" "${PICTPATH}${OUTPUT}" ;;
  '-f' ) grim -g "${FOCUSED}" "${PICTPATH}${OUTPUT}" ;;
  * | '-h') help=1
  	echo -e "Hyprland screenshot tool ver${Ver} by ${RED}Red${PURPLE}Nekod${BLUE}err${NORMAL}"
            ...
            echo ""
            echo ""
            echo "'-e' or empty to take a screenshot of the entire screen."
            echo "'-s' to take a screenshot of the selected area."
            echo "'-f' to take a screenshot of the focused window."
...
# 	notify-send "Screenshot" "File saved as <i>'$OUTPUT'</i> and copied to the clipboard." -i "${PICTPATH}${OUTPUT}"
	gdbus call --session --dest=org.freedesktop.Notifications --object-path=/org/freedesktop/Notifications --method=org.freedesktop.Notifications.Notify "" 0 "file://${PICTPATH}${OUTPUT}" 'Screenshot' "File saved and copied to clipboard." '[]' '{"urgency": <1>}' 5000
fi
